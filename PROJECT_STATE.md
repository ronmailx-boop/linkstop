...
- [x] **נמצא ותוקן באג נוסף (חשוב!)**: קישורי `facebook.com/share/...` החזירו דף גנרי "התחבר כדי לצפות" (בשפה לפי locale של ה-IP של הפונקציה - ראינו גרמנית, כי הפרויקט ב-`eu-central-1`) במקום התוכן האמיתי. הסיבה: פייסבוק חוסם User-Agent גנרי של בוט, אך **סומך על ה-crawler הרשמי שלו `facebookexternalhit`** (זה בדיוק מה שמאפשר תצוגה מקדימה בוואטסאפ/מסנג'ר). תוקן ע"י שינוי ה-`User-Agent` בפונקציה ל-`facebookexternalhit/1.1` (PR #12). **אומת בפועל** ע"י המשתמש - קישור `facebook.com/share/1GW1FHCB8E/` הציג כותרת+תמונה נכונות אחרי התיקון. גרסת הפונקציה הנוכחית בפרודקשן: **v7** (נקייה, בלי דיבוג לוגים).
...
4. **בכל commit עתידי: להעלות את מספר "גרסה N" ב-`index.html`, ללא יוצא מן הכלל** - גם בשינויים שנראים "backend-only" (כמו Edge Function) וגם דיפלוי-דיבוג זמני. המשתמש ביקש זאת מפורשות; אל תנסה לשקול "אם זה רלוונטי" - פשוט להעלות בכל commit/deploy. גרסה נוכחית: **6**.
