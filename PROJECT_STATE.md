...
## מחיקה מרובה (2026-09-18)
- [x] **בקשת המשתמש**: אפשרות לבחור ולמחוק כמה קישורים בבת אחת.
- [x] נוסף `<div id="list-toolbar">` (כפתור "בחירה מרובה") ו-`<div id="selection-bar">` (checkbox "בחר הכל", מונה נבחרים, כפתורי "ביטול"/"מחק" - משתמש מחדש בסטיילים `.confirm-dialog__button` הקיימים) ל-`index.html`, בין שורת החיפוש לרשימה.
- [x] נוספו סטיילים ל-`src/css/style.css`: `.list-toolbar`, `.selection-bar` (+ תת-אלמנטים), `.link-card__checkbox`, וגם `.confirm-dialog__button:disabled` (לא היה סטייל למצב disabled קודם).
- [x] `src/js/storage.js` - נוספה `deleteLinks(ids)` (מחיקה מרובה בכתיבה אחת ל-localStorage, במקום N קריאות/כתיבות נפרדות).
- [x] `src/js/app.js` - מצב `selectionMode` + `Set` של `selectedIds`. במצב בחירה: כל כרטיס מקבל checkbox, האזור הראשי של הכרטיס הופך מ-`<a>` (שנפתח בלינק) ל-`<div>` שלחיצה עליו מסמנת/מבטלת סימון (מטרה גדולה יותר ללחיצה, לא רק ה-checkbox עצמו), וכפתורי עריכה/מחיקה הבודדים מוסתרים. "בחר הכל" פועל רק על הקישורים **הגלויים כרגע** (מכבד סינון חיפוש פעיל). מחיקה מרובה עוברת דרך אותו `confirmDialog` הקיים (עם הודעה שמזכירה את הכמות: "למחוק קישור אחד?" / "למחוק N קישורים?"), ובסיום חוזרת אוטומטית למצב תצוגה רגיל.
- [x] **נבדק בפועל בדפדפן (Playwright + Chromium headless)**: כניסה למצב בחירה מציגה checkbox לכל כרטיס; כפתור המחיקה מנוטרל כשלא נבחר כלום; בחירה דרך לחיצה על השורה או על ה-checkbox עצמו עובדת; "בחר הכל"/ביטול הכל עובד; מחיקה מרובה עם אישור מוחקת רק את הנבחרים ויוצאת ממצב בחירה אוטומטית; ביטול (מבלי למחוק) משאיר הכל כמו שהיה; "בחר הכל" בזמן סינון חיפוש פעיל מסמן רק את הקישורים המסוננים הגלויים (לא את כל הרשימה). אין שגיאות קונסול.
- [x] הועלה מספר גרסה ב-`index.html` מ-14 ל-**15**.
